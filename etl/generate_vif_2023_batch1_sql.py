...
SRC = Path("datos_base/Violencia/Violencia intrafamiliar/2023/violencia_intrafamiliar.xlsx")
MUNI_FILE = Path("catalogos/municipios/municipios- depto.txt")
START_INDEX = 0
ROW_LIMIT = 500
CHUNK_SIZE = 100

OUT_CAT_AGR = Path("sql/inserts/catalogos/087_tipo_agresion_intrafamiliar_2023.sql")
OUT_CAT_INV = Path("sql/inserts/catalogos/088_involucramiento_vif.sql")
OUT_TX = Path("sql/inserts/transaccional/116_vif_2023_batch1_parcial.sql")
OUT_TX_CHUNKS_DIR = Path("sql/inserts/transaccional/chunks_116")
...
def norm(value: str) -> str:
    ...
    for source, target in (("A", "A"), ("E", "E"), ("I", "I"), ("O", "O"), ("U", "U"), ("N", "N")):
...
def parse_date(y, m, d):
    yy = clamp(safe_int(y, 2023), 2008, 2026)
...
def map_tipo_agresion(raw: str) -> tuple[str, str]:
    code = clean(raw)
    if not code:
        return "TIPAGRE_SD", "Tipo agresion no especificado"
    code = re.sub(r"\D", "", code)
    if not code:
        code = "SD"
    return f"TIPAGRE_{code}", f"Fuente VIF 2023 codigo {code}"


def write_catalogs(df: pd.DataFrame):
    tipos = sorted({map_tipo_agresion(v)[0] for v in df["HEC_TIPAGRE"].dropna().unique()})
    if not tipos:
        tipos = ["TIPAGRE_SD"]
    ...
    for t in tipos:
        lines.append(
            ...
            f"SELECT {sqlq(t)}, 'Fuente VIF 2023' FROM RDB$DATABASE "
    ...
    for inv in ("Victima", "Agresor"):
        inv_lines.append(
            "INSERT INTO involucramiento (nombre, descripcion) "
            f"SELECT {sqlq(inv)}, 'Fuente VIF 2023' FROM RDB$DATABASE "
            f"WHERE NOT EXISTS (SELECT 1 FROM involucramiento WHERE UPPER(TRIM(nombre)) = UPPER({sqlq(inv)}));"
        )
    inv_lines.extend(["", "COMMIT;"])
    OUT_CAT_INV.write_text("\n".join(inv_lines), encoding="utf-8")


def block_sql(token: str, dept_name: str, muni_name: str, fecha_hecho: str, fecha_emision: str, sexo_vic: str, sexo_agr: str, vic_birth: str, agr_birth: str, quien_reporta: str, boleta: str, otras_vic: int, otros_agr: int, tipo_agre: str):
...
BEGIN
  IF (EXISTS(SELECT 1 FROM persona p WHERE p.nombres = 'VIF23V_{token}' AND p.apellidos = 'VICTIMA')) THEN EXIT;
  ...
  VALUES ('VIF23V_{token}', 'VICTIMA', {sqlq(vic_birth)}, NULL, :v_gen_vic, :v_orient, :v_grupo, :v_eciv, :v_alf, :v_nivel, :v_ubic, 0)
  RETURNING id_persona INTO v_victima;

  INSERT INTO persona (nombres, apellidos, fecha_nacimiento, cui, id_genero, id_orientacion_sexual, id_grupo_etnico, id_estado_civil, id_condicion_alfabetica, id_nivel_escolaridad, id_origen, es_extranjero)
  VALUES ('VIF23A_{token}', 'AGRESOR', {sqlq(agr_birth)}, NULL, :v_gen_agr, :v_orient, :v_grupo, :v_eciv, :v_alf, :v_nivel, :v_ubic, 0)
...
def split_chunks(sql_text: str, chunk_size: int):
    blocks = []
    cur = []
    in_block = False
...
def main():
    full_df = pd.read_excel(SRC, sheet_name="Base VIF 2023")
    total = len(full_df)
    ...
    for idx, row in df.iterrows():
        ...
        tipo_agre, _ = map_tipo_agresion(row.get("HEC_TIPAGRE"))
    ...
    OUT_TX.write_text(full_sql, encoding="utf-8")

    OUT_TX_CHUNKS_DIR.mkdir(parents=True, exist_ok=True)
    for old in OUT_TX_CHUNKS_DIR.glob("116_vif_2023_part_*.sql"):
        old.unlink()

    blocks = split_chunks(full_sql, CHUNK_SIZE)
    for i in range(0, len(blocks), CHUNK_SIZE):
        ...
        part = (i // CHUNK_SIZE) + 1
        out = OUT_TX_CHUNKS_DIR / f"116_vif_2023_part_{part:02d}.sql"
