...
SRC = Path("datos_base/Violencia/Hechos-Delicitivos/Necropsias/necropsias.xlsx")
MUNI_FILE = Path("catalogos/municipios/municipios- depto.txt")
START_INDEX = 0
ROW_LIMIT = 1000
CHUNK_SIZE = 250

OUT_CAT_CLASS = Path("sql/inserts/catalogos/073_clasificacion_delito_necropsias.sql")
OUT_CAT_DEL = Path("sql/inserts/catalogos/074_delito_necropsias.sql")
OUT_CAT_DEL_COM = Path("sql/inserts/catalogos/075_delito_cometido_necropsias.sql")
OUT_TX = Path("sql/inserts/transaccional/112_necropsias_batch1_parcial.sql")
OUT_TX_CHUNKS_DIR = Path("sql/inserts/transaccional/chunks_112")
...
def build_catalogs(df: pd.DataFrame):
    ...
    OUT_CAT_CLASS.write_text(
        "\n".join(
            [
                ...
                f"SELECT NULL, {sqlq(class_name)}, 'Fuente Necropsias 2023', 1 FROM RDB$DATABASE",
    ...
    causes = sorted({cut_utf8(clean(v), 150) for v in df["causa_muerte"].dropna().unique() if clean(v)})
    cause_code = {}
    del_lines = ["SET NAMES UTF8;", ""]
    for idx, value in enumerate(causes, start=1):
        code = f"NEC-DEL-{idx:03d}"
        cause_code[value] = code
        del_lines.append(
            "INSERT INTO delito (codigo, nombre, bien_juridico, activo) "
            f"SELECT {sqlq(code)}, {sqlq(value)}, 'Fuente Necropsias 2023', 1 FROM RDB$DATABASE "
...
def main():
    ...
    cause_code, class_name = build_catalogs(df)
    ...
    for _, row in df.iterrows():
        ...
        year_ing, month_ing, day_ing, fecha_ing = parse_date(row.get("año_ing"), row.get("mes_ing"), row.get("día_ing"))
        ...
        cause_raw = cut_utf8(clean(row.get("causa_muerte")), 150)

        if not dept_raw or not muni_raw or not cause_raw:
            skipped += 1
            continue
        ...
        dcode = cause_code.get(cause_raw)
        if not dcode:
            skipped += 1
            continue
        ...
        birth_year = year_ing - max(1, age)
        birth_date = f"{birth_year:04d}-06-15"
        pcode = f"NEC_{year_ing}_{corr:06d}"
        ...
        tx_blocks.append(
            ...
            f"BEGIN\n"
            f"  IF (EXISTS(SELECT 1 FROM persona p WHERE p.nombres = {sqlq(pcode)} AND p.apellidos = 'NECROPSIAS')) THEN EXIT;\n"
            f"\n"
            ...
            f"  VALUES ({sqlq(pcode)}, 'NECROPSIAS', {sqlq(birth_date)}, NULL, :v_genero, :v_orient, :v_grupo, :v_eciv, :v_alf, :v_nivel, :v_ubic, 0)\n"
            ...
            f"  INSERT INTO hecho (id_tipo_hecho, id_ubicacion, fecha_hecho) VALUES (:v_tipo_hecho, :v_ubic, {sqlq(fecha_ing)}) RETURNING id_hecho INTO v_hecho;\n"
            f"\n"
    ...
    for old in OUT_TX_CHUNKS_DIR.glob("112_necropsias_part_*.sql"):
        old.unlink()
    ...
    for i in range(0, len(tx_blocks), CHUNK_SIZE):
        ...
        (OUT_TX_CHUNKS_DIR / f"112_necropsias_part_{chunk_count:02d}.sql").write_text("\n".join(chunk_lines), encoding="utf-8")
    ...
    pending_lines = [
        ...
        "- Siguiente accion sugerida: generar `112_necropsias_batch2_parcial.sql` empezando en `START_INDEX = " + str(end_idx) + "`.",
        "",
    ]
