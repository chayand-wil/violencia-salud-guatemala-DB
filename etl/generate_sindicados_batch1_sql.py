...
SRC = Path("datos_base/Violencia/Hechos-Delicitivos/Sindicatos/sindicados.xlsx")
MUNI_FILE = Path("catalogos/municipios/municipios- depto.txt")
START_INDEX = 0
ROW_LIMIT = 1000
CHUNK_SIZE = 250

OUT_CAT_INV = Path("sql/inserts/catalogos/072_involucramiento_sindicado.sql")
OUT_CAT_CLASS = Path("sql/inserts/catalogos/069_clasificacion_delito_sindicados.sql")
OUT_CAT_DEL = Path("sql/inserts/catalogos/070_delito_sindicados.sql")
OUT_CAT_DEL_COM = Path("sql/inserts/catalogos/071_delito_cometido_sindicados.sql")
OUT_TX = Path("sql/inserts/transaccional/111_sindicados_batch1_parcial.sql")
OUT_TX_CHUNKS_DIR = Path("sql/inserts/transaccional/chunks_111")
...
def build_catalogs(df: pd.DataFrame):
    ...
    for value in classes:
        class_lines.append(
            ...
            f"SELECT NULL, {sqlq(value)}, 'Fuente Sindicados 2023', 1 FROM RDB$DATABASE "
    ...
    for idx, value in enumerate(delicts, start=1):
        code = f"SIN-DEL-{idx:03d}"
        delito_code[value] = code
        del_lines.append(
            "INSERT INTO delito (codigo, nombre, bien_juridico, activo) "
            f"SELECT {sqlq(code)}, {sqlq(value)}, 'Fuente Sindicados 2023', 1 FROM RDB$DATABASE "
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        age = safe_int(row.get("edad_sind"), 30)
        ...
        pcode = f"SIN_{year_den}_{corr:06d}"
        ...
        tx_blocks.append(
            ...
            f"BEGIN\n"
            f"  IF (EXISTS(SELECT 1 FROM persona p WHERE p.nombres = {sqlq(pcode)} AND p.apellidos = 'SINDICADOS')) THEN EXIT;\n"
            f"\n"
            ...
            f"  VALUES ({sqlq(pcode)}, 'SINDICADOS', {sqlq(birth_date)}, NULL, :v_genero, :v_orient, :v_grupo, :v_eciv, :v_alf, :v_nivel, :v_ubic, 0)\n"
            ...
            f"\n"
            f"  SELECT id_involucramiento FROM involucramiento WHERE UPPER(TRIM(nombre)) = UPPER('Sindicado') ROWS 1 INTO v_inv;\n"
            f"  IF (v_inv IS NULL) THEN SELECT id_involucramiento FROM involucramiento WHERE UPPER(TRIM(nombre)) = UPPER('Detenido') ROWS 1 INTO v_inv;\n"
    ...
    for old in OUT_TX_CHUNKS_DIR.glob("111_sindicados_part_*.sql"):
        old.unlink()
    ...
    for i in range(0, len(tx_blocks), CHUNK_SIZE):
        ...
        (OUT_TX_CHUNKS_DIR / f"111_sindicados_part_{chunk_count:02d}.sql").write_text("\n".join(chunk_lines), encoding="utf-8")
    ...
    pending_lines = [
        ...
        "- Siguiente accion sugerida: generar `111_sindicados_batch2_parcial.sql` empezando en `START_INDEX = " + str(end_idx) + "`.",
        "",
    ]
