...
SRC = Path("datos_base/Violencia/Hechos-Delicitivos/PNC -Victimas/pnc_victimas.xlsx")
MUNI_FILE = Path("catalogos/municipios/municipios- depto.txt")

# Carga parcial para avanzar sin bloquear ejecucion.
START_INDEX = 0
ROW_LIMIT = 15000
CHUNK_SIZE = 1000

OUT_CAT_INV = Path("sql/inserts/catalogos/065_involucramiento_pnc_victima.sql")
OUT_CAT_CLASS = Path("sql/inserts/catalogos/062_clasificacion_delito_pnc_victimas.sql")
OUT_CAT_DEL = Path("sql/inserts/catalogos/063_delito_pnc_victimas.sql")
OUT_CAT_DEL_COM = Path("sql/inserts/catalogos/064_delito_cometido_pnc_victimas.sql")
OUT_TX = Path("sql/inserts/transaccional/109_pnc_victimas_batch1_parcial.sql")
OUT_TX_CHUNKS_DIR = Path("sql/inserts/transaccional/chunks_109")
...
def build_catalogs(df: pd.DataFrame):
    ...
    for c in classes:
        class_lines.append(
            ...
            f"SELECT NULL, {sqlq(c)}, 'Fuente PNC victimas 2023', 1 FROM RDB$DATABASE "
    ...
    for i, d in enumerate(delitos, start=1):
        code = f"PNCV-DEL-{i:03d}"
        delito_code[d] = code
        del_lines.append(
            "INSERT INTO delito (codigo, nombre, bien_juridico, activo) "
            f"SELECT {sqlq(code)}, {sqlq(d)}, 'Fuente PNC victimas 2023', 1 FROM RDB$DATABASE "
...
def main():
    ...
    for src_idx, row in df.iterrows():
        ...
        edad = max(0, min(95, edad))
        fnac = f"{year - max(1, edad):04d}-06-15"

        pcode = f"PNCVIC_{year}_{corr:06d}"
        ...
        tx_blocks.append(
            ...
            f"BEGIN\n"
            f"  IF (EXISTS(SELECT 1 FROM persona p WHERE p.nombres = {sqlq(pcode)} AND p.apellidos = 'PNC VICTIMAS')) THEN EXIT;\n"
            f"\n"
            ...
            f"  VALUES ({sqlq(pcode)}, 'PNC VICTIMAS', {sqlq(fnac)}, NULL, :v_genero, :v_orient, :v_grupo, :v_eciv, :v_alf, :v_nivel, :v_ubic, 0)\n"
    ...
    tx_header = [
        ...
        "SET TERM ^ ;",
        "",
        "-- Fuente: PNC victimas 2023 (carga parcial)",
        "-- Flujo: ubicacion -> persona -> hecho -> involucrado_hecho(agraviado) -> hecho_delictivo",
        "",
    ]
    tx_tail = ["SET TERM ; ^", "COMMIT;"]

    tx_lines = tx_header + [blk for b in tx_blocks for blk in (b, "")] + tx_tail
    OUT_TX.write_text("\n".join(tx_lines), encoding="utf-8")

    OUT_TX_CHUNKS_DIR.mkdir(parents=True, exist_ok=True)
    # Limpieza de chunks previos para evitar mezclar rangos distintos.
    for old in OUT_TX_CHUNKS_DIR.glob("109_pnc_victimas_part_*.sql"):
        old.unlink()
    ...
    for i in range(0, len(tx_blocks), CHUNK_SIZE):
        ...
        out_chunk = OUT_TX_CHUNKS_DIR / f"109_pnc_victimas_part_{chunk_count:02d}.sql"
        out_chunk.write_text("\n".join(chunk_lines), encoding="utf-8")
    ...
    pending_text = "\n".join(
        [
            ...
            "- Siguiente accion sugerida: generar `109_pnc_victimas_batch2_parcial.sql` empezando en `START_INDEX = "
            + str(end_idx)
            + "`.",
            "",
        ]
    )
    OUT_PENDING.write_text(pending_text, encoding="utf-8")
