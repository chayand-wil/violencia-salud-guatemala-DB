...
SRC = Path("datos_base/Violencia/Hechos-Delicitivos/Evaluacion Medicos - INACIF/medicos_inacif.xlsx")
MUNI_FILE = Path("catalogos/municipios/municipios- depto.txt")
START_INDEX = 0
ROW_LIMIT = 1000
CHUNK_SIZE = 250

OUT_CAT_CLASS = Path("sql/inserts/catalogos/079_clasificacion_delito_medicos_inacif.sql")
OUT_CAT_DEL = Path("sql/inserts/catalogos/080_delito_medicos_inacif.sql")
OUT_CAT_DEL_COM = Path("sql/inserts/catalogos/081_delito_cometido_medicos_inacif.sql")
OUT_TX = Path("sql/inserts/transaccional/114_medicos_inacif_batch1_parcial.sql")
OUT_TX_CHUNKS_DIR = Path("sql/inserts/transaccional/chunks_114")
...
def build_catalogs(df: pd.DataFrame):
    classes = sorted({cut_utf8(clean(v), 150) for v in df["clasif_eval"].dropna().unique() if clean(v)})
    if not classes:
        classes = ["Otros"]
    ...
    for value in classes:
        class_lines.append(
            ...
            f"SELECT NULL, {sqlq(value)}, 'Fuente Medicos INACIF 2023', 1 FROM RDB$DATABASE "
    ...
    for idx, value in enumerate(classes, start=1):
        code = f"MED-DEL-{idx:03d}"
        delito_code[value] = code
        del_lines.append(
            "INSERT INTO delito (codigo, nombre, bien_juridico, activo) "
            f"SELECT {sqlq(code)}, {sqlq(value)}, 'Fuente Medicos INACIF 2023', 1 FROM RDB$DATABASE "
    ...
    for value in classes:
        ...
        pair_lines.append(
            ...
            f"WHERE d.codigo = {sqlq(code)} AND UPPER(TRIM(c.nombre)) = UPPER({sqlq(value)}) "
            "AND NOT EXISTS ("
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        year_ocu, month_ocu, day_ocu, fecha = parse_date(row.get("año_ocu"), row.get("mes_ocu"), row.get("día_ocu"))
        ...
        class_raw = cut_utf8(clean(row.get("clasif_eval")), 150) or "Otros"
        if class_raw not in delito_code:
            class_raw = "Otros"
            if class_raw not in delito_code:
                skipped += 1
                continue

        dcode = delito_code[class_raw]
        sex_raw = norm(row.get("sexo_per"))
        age = safe_int(row.get("edad_per"), 30)
        gender = "Mujer" if sex_raw.startswith("M") else "Hombre" if sex_raw.startswith("H") else "Ignorado"
        age = max(0, min(95, age))
        birth_year = year_ocu - max(1, age)
        birth_date = f"{birth_year:04d}-06-15"
        pcode = f"MED_{year_ocu}_{corr:06d}"
        ...
        tx_blocks.append(
            ...
            f"BEGIN\n"
            f"  IF (EXISTS(SELECT 1 FROM persona p WHERE p.nombres = {sqlq(pcode)} AND p.apellidos = 'MEDICOS INACIF')) THEN EXIT;\n"
            f"\n"
            ...
            f"  VALUES ({sqlq(pcode)}, 'MEDICOS INACIF', {sqlq(birth_date)}, NULL, :v_genero, :v_orient, :v_grupo, :v_eciv, :v_alf, :v_nivel, :v_ubic, 0)\n"
    ...
    for old in OUT_TX_CHUNKS_DIR.glob("114_medicos_inacif_part_*.sql"):
        old.unlink()
    ...
    for i in range(0, len(tx_blocks), CHUNK_SIZE):
        ...
        (OUT_TX_CHUNKS_DIR / f"114_medicos_inacif_part_{chunk_count:02d}.sql").write_text("\n".join(chunk_lines), encoding="utf-8")
    ...
    pending_lines = [
        ...
        "- Siguiente accion sugerida: generar `114_medicos_inacif_batch2_parcial.sql` empezando en `START_INDEX = " + str(end_idx) + "`.",
        "",
    ]
