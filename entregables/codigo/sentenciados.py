...
SRC = Path("datos_base/Violencia/Hechos-Delicitivos/Organismo judical - Sentenciados/sentenciados.xlsx")
MUNI_FILE = Path("catalogos/municipios/municipios- depto.txt")
START_INDEX = 0
ROW_LIMIT = 1000
CHUNK_SIZE = 250

OUT_CAT_CLASS = Path("sql/inserts/catalogos/082_clasificacion_delito_sentenciados.sql")
OUT_CAT_DEL = Path("sql/inserts/catalogos/083_delito_sentenciados.sql")
OUT_CAT_DEL_COM = Path("sql/inserts/catalogos/084_delito_cometido_sentenciados.sql")
OUT_CAT_INV = Path("sql/inserts/catalogos/085_involucramiento_sentenciados.sql")
OUT_CAT_FALLO = Path("sql/inserts/catalogos/086_tipo_fallo_sentenciados.sql")
OUT_TX = Path("sql/inserts/transaccional/115_sentenciados_batch1_parcial.sql")
OUT_TX_CHUNKS_DIR = Path("sql/inserts/transaccional/chunks_115")
...
def parse_reg_date(year_value, month_value):
...
def build_catalogs(df: pd.DataFrame):
    classes = []
    class_seen = set()
    delicts = sorted({cut_utf8(clean(v), 150) for v in df["delito_cod"].dropna().unique() if clean(v)})
    invols = sorted({cut_utf8(clean(v), 60) for v in df["Involucramiento"].dropna().unique() if clean(v)})
    fallos = sorted({cut_utf8(clean(v), 80) for v in df["tip_fallo"].dropna().unique() if clean(v)})

    for _, row in df.iterrows():
        ley = cut_utf8(clean(row.get("tip_ley")), 80)
        titulo = cut_utf8(clean(row.get("título")), 80)
        capitulo = cut_utf8(clean(row.get("capítulo")), 80)
        label = " | ".join([p for p in [ley, titulo, capitulo] if p])
        if not label:
            label = "SIN CLASIFICACION"
        label = cut_utf8(label, 150)
        if label not in class_seen:
            class_seen.add(label)
            classes.append(label)

    if not classes:
        classes = ["SIN CLASIFICACION"]

    class_lines = ["SET NAMES UTF8;", ""]
    for value in classes:
        class_lines.append(
            "INSERT INTO clasificacion_delito (codigo, nombre, descripcion, activo) "
            f"SELECT NULL, {sqlq(value)}, 'Fuente Sentenciados 2023', 1 FROM RDB$DATABASE "
    ...
    for idx, value in enumerate(delicts, start=1):
        code = f"SEN-DEL-{idx:03d}"
        delito_code[value] = code
        del_lines.append(
            "INSERT INTO delito (codigo, nombre, bien_juridico, activo) "
            f"SELECT {sqlq(code)}, {sqlq(value)}, 'Fuente Sentenciados 2023', 1 FROM RDB$DATABASE "
    ...
    pair_seen = set()
    for _, row in df.iterrows():
        delito_name = cut_utf8(clean(row.get("delito_cod")), 150)
        if not delito_name:
            continue
        dcode = delito_code.get(delito_name)
        if not dcode:
            continue
        ley = cut_utf8(clean(row.get("tip_ley")), 80)
        titulo = cut_utf8(clean(row.get("título")), 80)
        capitulo = cut_utf8(clean(row.get("capítulo")), 80)
        class_name = " | ".join([p for p in [ley, titulo, capitulo] if p])
        if not class_name:
            class_name = "SIN CLASIFICACION"
        class_name = cut_utf8(class_name, 150)
        key = (dcode, class_name)
        if key in pair_seen:
            continue
        pair_seen.add(key)
        pair_lines.append(
            ...
            f"WHERE d.codigo = {sqlq(dcode)} AND UPPER(TRIM(c.nombre)) = UPPER({sqlq(class_name)}) "
            "AND NOT EXISTS ("
    ...
    inv_lines = ["SET NAMES UTF8;", ""]
    for value in invols:
        inv_lines.append(
            "INSERT INTO involucramiento (nombre, descripcion) "
            f"SELECT {sqlq(value)}, 'Fuente Sentenciados 2023' FROM RDB$DATABASE "
            f"WHERE NOT EXISTS (SELECT 1 FROM involucramiento WHERE UPPER(TRIM(nombre)) = UPPER({sqlq(value)}));"
        )
    inv_lines.extend(["", "COMMIT;"])
    OUT_CAT_INV.write_text("\n".join(inv_lines), encoding="utf-8")

    fallo_lines = ["SET NAMES UTF8;", ""]
    for value in fallos:
        fallo_lines.append(
            "INSERT INTO tipo_fallo (nombre, descripcion) "
            f"SELECT {sqlq(value)}, 'Fuente Sentenciados 2023' FROM RDB$DATABASE "
            f"WHERE NOT EXISTS (SELECT 1 FROM tipo_fallo WHERE UPPER(TRIM(nombre)) = UPPER({sqlq(value)}));"
        )
    fallo_lines.extend(["", "COMMIT;"])
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        year_reg, month_reg, _, fecha = parse_reg_date(row.get("año_reg"), row.get("mes_reg"))

        dept_raw = clean(row.get("depto_reg"))
        ...
        delito_raw = cut_utf8(clean(row.get("delito_cod")), 150)
        if not delito_raw:
            skipped += 1
            continue
        ...
        titulo = cut_utf8(clean(row.get("título")), 80)
        capitulo = cut_utf8(clean(row.get("capítulo")), 80)
        class_raw = " | ".join([p for p in [ley, titulo, capitulo] if p])
        if not class_raw:
            class_raw = "SIN CLASIFICACION"
        class_raw = cut_utf8(class_raw, 150)

        sex_raw = norm(row.get("sexo"))
        men_may = norm(row.get("men_may"))
        nationality_raw = cut_utf8(clean(row.get("nacionalidad")), 80)
        inv_raw = cut_utf8(clean(row.get("Involucramiento")), 60)
        age = 35 if men_may.startswith("MAY") else 16
        is_foreigner = 1 if nationality_raw and "GUATEM" not in norm(nationality_raw) else 0

        gender = "Mujer" if sex_raw.startswith("MUJ") else "Hombre" if sex_raw.startswith("HOM") else "Ignorado"
        age = max(0, min(95, age))
        birth_year = year_reg - max(1, age)
        birth_date = f"{birth_year:04d}-06-15"
        pcode = f"SEN_{year_reg}_{corr:06d}"
        ...
        tx_blocks.append(
            ...
            f"BEGIN\n"
            f"  IF (EXISTS(SELECT 1 FROM persona p WHERE p.nombres = {sqlq(pcode)} AND p.apellidos = 'SENTENCIADOS OJ')) THEN EXIT;\n"
            f"\n"
            ...
            f"  VALUES ({sqlq(pcode)}, 'SENTENCIADOS OJ', {sqlq(birth_date)}, NULL, :v_genero, :v_orient, :v_grupo, :v_eciv, :v_alf, :v_nivel, :v_ubic, {is_foreigner})\n"
            ...
            f"\n"
            f"  SELECT id_involucramiento FROM involucramiento WHERE UPPER(TRIM(nombre)) = UPPER({sqlq(inv_raw)}) ROWS 1 INTO v_inv;\n"
    ...
    for old in OUT_TX_CHUNKS_DIR.glob("115_sentenciados_part_*.sql"):
        old.unlink()
    ...
    for i in range(0, len(tx_blocks), CHUNK_SIZE):
        ...
        (OUT_TX_CHUNKS_DIR / f"115_sentenciados_part_{chunk_count:02d}.sql").write_text("\n".join(chunk_lines), encoding="utf-8")
    ...
    pending_lines = [
        ...
        "- Fuente: datos_base/Violencia/Hechos-Delicitivos/Exhumaciones/exhumaciones.xlsx",
        "- Total filas fuente: 118",
        "- Rango cargado en parcial: 1 a 60",
        "- Rango pendiente: 61 a 118",
        "- Chunks generados para ejecucion segura: 3 (directorio: sql/inserts/transaccional/chunks_113)",
        "- Siguiente accion sugerida: generar `113_exhumaciones_batch2_parcial.sql` empezando en `START_INDEX = 60`.",
        "",
        "## Batch 114 - Medicos INACIF",
        "- Fuente: datos_base/Violencia/Hechos-Delicitivos/Evaluacion Medicos - INACIF/medicos_inacif.xlsx",
        "- Total filas fuente: 148537",
        "- Rango cargado en parcial: 1 a 1000",
        "- Rango pendiente: 1001 a 148537",
        "- Chunks generados para ejecucion segura: 4 (directorio: sql/inserts/transaccional/chunks_114)",
        "- Siguiente accion sugerida: generar `114_medicos_inacif_batch2_parcial.sql` empezando en `START_INDEX = 1000`.",
        "",
        "## Batch 115 - Sentenciados",
        "- Fuente: datos_base/Violencia/Hechos-Delicitivos/Organismo judical - Sentenciados/sentenciados.xlsx",
        f"- Total filas fuente: {total}",
        f"- Rango cargado en parcial: {START_INDEX + 1} a {end_idx}",
        f"- Rango pendiente: {end_idx + 1} a {total}" if end_idx < total else "- Rango pendiente: completo",
        f"- Chunks generados para ejecucion segura: {chunk_count} (directorio: sql/inserts/transaccional/chunks_115)",
        f"- Siguiente accion sugerida: generar `115_sentenciados_batch2_parcial.sql` empezando en `START_INDEX = {end_idx}`." if end_idx < total else "- Siguiente accion sugerida: batch 115 completo.",
    ]
    OUT_PENDING.write_text("\n".join(pending_lines) + "\n", encoding="utf-8")

    print(f"Filas fuente totales: {total}")
    print(f"Rango cargado: {START_INDEX + 1}..{end_idx}")
    print(f"Filas omitidas por validaciones: {skipped}")
    print(f"Bloques transaccionales: {len(tx_blocks)}")
    print(f"Chunks escritos: {chunk_count}")
